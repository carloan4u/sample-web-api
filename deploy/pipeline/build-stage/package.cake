...
Task("TransformQaConfig")
    .Does(() =>
{
   var secretConfigItem = EnvironmentVariable("QA-SecretValue") ??  "UNKNOWN";
   TransformConfig(@"./deploy/config/set-parameters-qa.xml", 
        new TransformationCollection {
            { "parameters/setParameter[@name='SecretValue']/@value", secretConfigItem }
        });
});
...
Task("TransformProdConfig")
    .Does(() =>
{
   var secretConfigItem = EnvironmentVariable("PROD-SecretValue") ??  "UNKNOWN";
   TransformConfig(@"./deploy/config/set-parameters-prod.xml", 
        new TransformationCollection {
            { "parameters/setParameter[@name='SecretValue']/@value", secretConfigItem }
        });
});
