terraform {
  backend "s3" {
    ...
    key    = "services/kierans-sample-app/qa.tfstate"
    region = "eu-west-2"
...
resource "aws_elastic_beanstalk_application" "default" {
  name        = "kierans-sample-app"
  description = "Kierans Sample App"
}
...
resource "aws_cloudwatch_metric_alarm" "kieran-test-alarm" {
  count = "${length(module.beanstalk-web-app.instances)}"
  alarm_name                = "kieran-test-alarm${count.index + 1}"
  ...
  alarm_actions     = ["arn:aws:sns:eu-west-2:276973094769:kierans-sample-app-topic"]
}