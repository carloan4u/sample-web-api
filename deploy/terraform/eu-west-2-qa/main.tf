...
resource "aws_elastic_beanstalk_application" "default" {
  name        = "dc-workshop-app"
  description = "DC Workshop App"
}
...
resource "aws_cloudwatch_metric_alarm" "dc-test-alarm" {
  count = "${length(module.beanstalk-web-app.instances)}"
  alarm_name                = "dc-test-alarm${count.index + 1}"
  ...
  alarm_actions     = ["arn:aws:sns:eu-west-2:276973094769:dc-workshop-app-qa"]
}
