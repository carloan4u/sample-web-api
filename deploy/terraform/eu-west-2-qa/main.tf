terraform {
  backend "s3" {
    ...
    key    = "services/clifton-sample-app/qa.tfstate"
    region = "eu-west-2"
...
resource "aws_elastic_beanstalk_application" "default" {
  name        = "clifton-sample-app"
  description = "JC Sample App"
}
...
module "beanstalk-web-app" {
  ...
  owner_tag         = "Pricing-Product-Selection"
...
resource "aws_cloudwatch_metric_alarm" "test-alarm" {
  ...
  alarm_name                = "terraform-clifton-test-alarm${count.index + 1}"
  ...
  alarm_actions     = ["arn:aws:sns:eu-west-2:276973094769:clifton-sample-app-Alert"]
}