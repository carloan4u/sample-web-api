terraform {
  backend "s3" {
    ...
    key    = "services/robapp/qa.tfstate"
    region = "eu-west-2"
...
resource "aws_elastic_beanstalk_application" "default" {
  name        = "robapp"
  description = "Rob test app service"
}
...
resource "aws_cloudwatch_metric_alarm" "rob-test-alarm" {
  count = "${length(module.beanstalk-web-app.instances)}"
  alarm_name                = "rob-test-alarm${count.index + 1}"
  ...
  alarm_actions     = ["arn:aws:sns:eu-west-2:276973094769:robs-topic"]
}
